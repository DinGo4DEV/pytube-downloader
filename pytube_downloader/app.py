...
import validators
from models.yt import PyTubeDonwloader
import time
import json
...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.entry = customtkinter.CTkEntry(self, placeholder_text="youtube video url or playlist")
        self.entry.grid(row=3, column=1, columnspan=2, padx=(20, 0), pady=(20, 20), sticky="nsew")

        self.main_button_1 = customtkinter.CTkButton(master=self, text="Download", fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"),command=self.donwload_button_event)
        ...
        with PyTubeDonwloader() as ydl:
            ...
            if _type:    
                ...
                with ThreadPoolExecutor( ) as exc:
                    thumbnails =  map(lambda v: v['thumbnails'][0],self.videos)
                    images = exc.map(get_thumbnails,thumbnails)
                ...
                for content in images:
                    ...
                    i = customtkinter.CTkImage(light_image=image,
                                  dark_image=image,
                                  size=(120, 120))
                    self.videos.append(customtkinter.CTkButton(self.tabview.tab("CTkTabview"), image=i))
                print(self.videos)
                
                
            else:
                thumbnails = ydl.list_thumbnails(info_dict)
            # for key,value in info_dict.items():                
            #     self.textbox.insert(tkinter.END,f"{key}:{value}\n")                                    
                # self.after(1000,self.textbox.update_idletasks)
